...
public class CanvasRobot
{
    ...
    // Parcourir tous les robots dans le monde
    ArrayList<Robot> robots = WorldOfRobot.listRobot();
    
    // Comparer les positions des robots entre eux pour détecter une collision
    for (int i = 0; i < robots.size(); i++) {
        ...
        for (int j = i + 1; j < robots.size(); j++) {
            ...
            // Vérifier si les deux robots sont aux mêmes coordonnées
            if (robot1.getXPosition() == robot2.getXPosition() && robot1.getYPosition() == robot2.getYPosition()) {
                // Si collision, changer la direction des deux robots
                handleCollision(robot1);
                handleCollision(robot2);
            }
        }
    }
}

private static void handleCollision(Robot robot) {
    // Changer de direction en fonction du type de robot
    if (robot instanceof VerticalRobot) {
        // Si c'est un robot vertical, inverser la direction : Nord <-> Sud
        if (robot.getYPosition() >= robot.getMax()) {
            robot.moveSouth();
        } else if (robot.getYPosition() <= robot.getMin()) {
            robot.moveNorth();
        } else {
            // Si pas à la limite, choisir aléatoirement une direction opposée
            if (Math.random() < 0.5) {
                robot.moveNorth();
            } else {
                robot.moveSouth();
        ...
        // Si c'est un robot horizontal, inverser la direction : Est <-> Ouest
        if (robot.getXPosition() >= robot.getMax()) {
            robot.moveWest();
        } else if (robot.getXPosition() <= robot.getMin()) {
            robot.moveEast();
        } else {
            // Si pas à la limite, choisir aléatoirement une direction opposée
            if (Math.random() < 0.5) {
